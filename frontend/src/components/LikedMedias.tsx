...
export default function LikedMedias() {
  const [token] = useLocalStorage('token', null);
  const { data: medias, error } = useSWR(['http://spotiflyx.xyz:8443/likes/me', token], fetcher);

  if (error) return <div>Error: {error.message}</div>;
  if (!medias) return <h1 className="text-3xl">Liked medias</h1>;

  return (
    <main className="mt-4 flex flex-wrap gap-8 justify-evenly">
      <section className='mx-8'>
        <h1 className="text-3xl text-center mb-8">Liked videos</h1>
      ...
      <section className='mx-8'>
        <h1 className="text-3xl text-center mb-8">Liked musics</h1>
